...
public class FloatingPhysicsCSharp : MonoBehaviour {
	...
	void FixedUpdate () {   //FixedUpdate is used because it is used with physics
	    ...
	    GetComponent<Rigidbody>().velocity = new Vector3(xMovement, Mathf.Sin(3*sine*verticalSpeed)*verticalDistance, 0);	//Adds the x axis movement and up and down motion to the object
